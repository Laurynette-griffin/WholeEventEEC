...
vector<PseudoJet> thermalGen(int nParticles = 300){
...
int main(int argc, char* argv[]) {
    ...
    TH1F EEC_ms("EEC_ms", "Energy Energy Correlator", bins, eecbounds); // mix signal
    TH1F EEC_mm("EEC_mm", "Energy Energy Correlator", bins, eecbounds); // mix1 mix1 
    TH1F EEC_m2("EEC_m2", "Energy Energy Correlator", bins, eecbounds); // mix1 mix2
 
    //phi + thermal components
    TH1F EEC_wp("EEC_wp", "Energy Energy Correlator", bins, topi);
    TH1F EEC_tp("EEC_tp", "Energy Energy Correlator", bins, topi);
    TH1F EEC_mp("EEC_mp", "Energy Energy Correlator", bins, topi);
    TH1F EEC_sp("EEC_sp", "Energy Energy Correlator", bins, topi);
    
    TH1F EEC_msp("EEC_msp", "Energy Energy Correlator", bins, topi); // mix signal phi
    TH1F EEC_mmp("EEC_mmp", "Energy Energy Correlator", bins, topi); // mix1 mix1 phi 
    TH1F EEC_m2p("EEC_m2p", "Energy Energy Correlator", bins, topi); // mix1 mix2 phi
    ...
    for (int iEvent = 0; iEvent < 1000000; ++iEvent) { // 1M events
       ...
       // pT cut > .2 GeV
        ...
        vector<PseudoJet> thermalParticles = thermalGen(300);
        
        vector<PseudoJet> m1 = thermalGen(300);
        vector<PseudoJet> m2 = thermalGen(300);
        ...
        for (size_t i = 0; i < charged_event.size(); ++i) {
            ...
            for (size_t k = 0; k < m1.size(); ++k) {
                ...
                double sm_delphi = deltaphi(charged_event[i], m1[k]);
            ...
            for (size_t j = i + 1; j < charged_event.size(); ++j) {
                ...
                // filling thermal, signal, and mixed hists from regular eec 
                if (ui == 1 && uj == 1)  EEC_t.Fill(z, eec/pmq2);
                else if (ui == -1 &&  uj == -1) EEC_s.Fill(z, eec/pmq2);
                else  EEC_m.Fill(z, eec/pmq2);
                
                EEC_wp.Fill(delphi, eec/pmq2);
                
                if (ui == 1 && uj == 1)  EEC_tp.Fill(delphi, eec/pmq2);
                else if (ui == -1 &&  uj == -1) EEC_sp.Fill(delphi, eec/pmq2);
                else  EEC_mp.Fill(delphi, eec/pmq2);
