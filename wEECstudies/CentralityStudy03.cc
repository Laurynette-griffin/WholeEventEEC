...
int main(int argc, char* argv[]) {
    ...
    TH1D LeadingJetSpectrum("LeadingJetspectrum", "Leading Jet Spectrum", 70 , 0, 70);
    TH1D SubleadingJetSpectrum("SubleadingJetSpectrum", "Subleading Jet Spectrum ", 70, 0, 70);
    TH1D ZSpectrum("zspectrum", "z Spectrum ", 100, 0.0, 1.0);
    
    TH2D qvspmq("comparisonpthat", "pthat^2 vs Average leading and subleading jet pt^2", 12, 0, 60, 10, 0, 60);
    TH3D ptvetaphit("ptvetaphi", "pt vs delphi", 32, -2*TMath::Pi(), 2*TMath::Pi(), 22, -1.1, 1.1, 10, 0, 10); 
    ...
    for (int iEvent = 0; iEvent < 500000; ++iEvent) { // 1M events
       ...
       // pT cut > .2 GeV
        ...
        for (size_t i = 0; i < thermalParticles.size(); ++i) {
            ptvetaphit.Fill(thermalParticles.at(i).phi(), thermalParticles.at(i).eta(), thermalParticles.at(i).pt());
            avthermalpt += thermalParticles.at(i).pt();
        }
        
        avthermalpt = avthermalpt / 1500;
        
        cout << "Average thermal pt" << avthermalpt << endl;
        // make 2 more for mixed event 
         //m1 first 
        ...
        for (size_t i = 0; i < charged_event.size(); ++i) {
            ...
            for (size_t j = i + 1; j < charged_event.size(); ++j) {
                ...
                if (ui == 1 && uj == 1)  EEC_t.Fill(z, eec/pmq2); // thermal
                if (ui == -1 &&  uj == -1) EEC_s.Fill(z, eec/pmq2); //signal
                if ((ui == -1 &&  uj == 1) || (ui == 1 &&  uj == -1 )) EEC_m.Fill(z, eec/pmq2); //mixed
                
                EEC_wp.Fill(delphi, eec/pmq2);
                
                if (ui == 1 && uj == 1)  EEC_tp.Fill(delphi, eec/pmq2);
                if (ui == -1 &&  uj == -1) EEC_sp.Fill(delphi, eec/pmq2);
                if ((ui == -1 &&  uj == 1) || (ui == 1 &&  uj == -1 ))  EEC_mp.Fill(delphi, eec/pmq2);
            }
