...
int main(int argc, char* argv[]) {
    ...
    // phi linear bins 
    TH1D EEC_wpl("EEC_wpl", "Energy Energy Correlator", 60, 0,  M_PI);
    TH1D EEC_tpl("EEC_tpl", "Energy Energy Correlator", 60, 0,  M_PI);
    TH1D EEC_mpl("EEC_mpl", "Energy Energy Correlator", 60, 0,  M_PI);
    TH1D EEC_spl("EEC_spl", "Energy Energy Correlator", 60, 0,  M_PI);
    
    TH1D EEC_mspl("EEC_mspl", "Energy Energy Correlator", 60, 0, M_PI); // mix signal phi
    TH1D EEC_mmpl("EEC_mmpl", "Energy Energy Correlator", 60, 0,  M_PI); // mix1 mix1 phi 
    TH1D EEC_m2pl("EEC_m2pl", "Energy Energy Correlator", 60, 0,  M_PI); // mix1 mix2 phi
    ...
    for (int iEvent = 0; iEvent < 500000; ++iEvent) { // 1M events
       ...
       // pT cut > .2 GeV
        ...
        if (!p.isFinal() || p.pT() < 2.0 || !p.isCharged() || abs(p.eta()) > 1.1 ) continue;
        ...
        thermalParticles.erase(std::remove_if(thermalParticles.begin(), thermalParticles.end(),
            [](const fastjet::PseudoJet& jet) { 
                return jet.pt() < 2; 
            }),
        thermalParticles.end());
        
        vector<PseudoJet> m1 = thermalGen(1500);
        
        
        m1.erase(std::remove_if(m1.begin(), m1.end(),
            [](const fastjet::PseudoJet& jet) { 
                return jet.pt() < 2; 
            }),
        m1.end());
...
;                }
        ...
        m2.erase(std::remove_if(m2.begin(), m2.end(),
            [](const fastjet::PseudoJet& jet) { 
                return jet.pt() < 2; 
            }),
        m2.end());
