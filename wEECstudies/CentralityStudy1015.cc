...
// pthatmin = 8 in .cmnd pthatmin = 70
...
double_t deltaRBinsEEC[nDeltaRBinsEEC + 1];
 ...
 // its not used here :)    
vector<PseudoJet> thermalGen(int nParticles = 850){
...
int main(int argc, char* argv[]) {
    ...
    // whole event 
    TH1F EEC_w("EEC_w", "Energy Energy Correlalor", bins, eecbounds);
    TH1F EEC_l("EEC_l", "Energy Energy Correlator", bins, eecbounds);
    TH1F EEC_m("EEC_m", "Energy Energy Correlator", bins, eecbounds);
    TH1F EEC_h("EEC_h", "Energy Energy Correlator", bins, eecbounds);
    
   //phi
    TH1F EEC_wp("EEC_wp", "Energy Energy Correlator", bins, topi);
    TH1F EEC_lp("EEC_lp", "Energy Energy Correlator", bins, topi);
    TH1F EEC_mp("EEC_mp", "Energy Energy Correlator", bins, topi);
    TH1F EEC_hp("EEC_hp", "Energy Energy Correlator", bins, topi);
       ...
       // pT cut > .2 GeV
        ...
        for (size_t i = 0; i < charged_event.size(); ++i) {
            for (size_t j = i + 1; j < charged_event.size(); ++j) {
                ...
                double avejet = (jets[0].pt() + jets[1].pt()) / 2;
                double pmq2 = pow(avejet, 2); // average of leading jets pt squared 
                double z = (1 - ctheta)/2; 
                ...
                if (jets[0].pt() >= 20.9 && jets[0].pt() < 31.2) EEC_l.Fill(z, eec/pmq2);
                if (jets[0].pt() >= 31.2 && jets[0].pt() < 40.7) EEC_m.Fill(z, eec/pmq2);
                if (jets[0].pt() >= 40.7 && jets[0].pt() < 60.8) EEC_h.Fill(z, eec/pmq2);
                
                EEC_wp.Fill(delphi, eec/pmq2);
                
                if (jets[0].pt() >= 20.9 && jets[0].pt() < 31.2) EEC_lp.Fill(delphi, eec/pmq2);
                if (jets[0].pt() >= 31.2 && jets[0].pt() < 40.7) EEC_mp.Fill(delphi, eec/pmq2);
                if (jets[0].pt() >= 40.7 && jets[0].pt() < 60.8) EEC_hp.Fill(delphi, eec/pmq2);
