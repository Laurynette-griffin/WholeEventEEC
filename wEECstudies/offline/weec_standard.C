...
void weec_standard() {
    ...
    //vector<const char*> histonames = {"EEC_z", "EEC_theta", "EEC_corr"}; 
    //vector<const char*> histonames = {"EEC_t1t1", "EEC_t2t2", "EEC_t3t3", "EEC_t1t2", "EEC_t1t3", "EEC_t2t3", "EEC_s"};
    //vector<const char*> histonames = {"EEC_w", "EEC_s"}; 
    ...
    string outputFilename = "../plots/weecstandarddelphiDec16.C";
    string outputFilename1 = "../plots/weecstandardelphiDec16.png";
    ...
    for (int i = 0; i < number; ++i) {
        ...
        // literally get rid of this 
        
        // Normalize to bin width
        if(i == 20){
            for (int bin = 1; bin <= histos[i]->GetNbinsX(); ++bin) {
                if (bin == 20){
                    double tl = histos[1]->GetBinLowEdge(bin);
                    double tr = histos[1]->GetBinLowEdge(bin+1);
                    double t = sqrt(tl*tr);
                    //double t = acos(1-2*z);
                    double c = sin(t)/2;
                    ...
                    histos[i]->SetBinContent(bin, c* content / width);
                }
                else if(bin == 21){
                    double tl = histos[1]->GetBinLowEdge(bin);
                    double tr = histos[1]->GetBinLowEdge(bin+1);
                    double tp = sqrt(tl*tr);
                    double ta = (tl+tr)/2;
                    double td = ta - tp;
                    double t = ta + td;
                    cout << t << endl;
                /*
                    double zl = histos[i]->GetBinLowEdge(bin);
                    double zr = histos[i]->GetBinLowEdge(bin+1);
                    double z = sqrt(zl*zr);
                    double t = acos(1-2*z);
                    */
                    double c = sin(t)/2;
                    ...
                    double t = histos[1]->GetBinCenter(bin);
                    //double zr = histos[1]->GetBinLowEdge(bin+1);
                    //double z = pow(10.0, log10(zl) +(bin + (zr/zl))* ((log10(zr)-log10(zl))/bins));
                    //double z = pow(10.0, (log10(zl)+log10(zr))/2);
                    //double z = sqrt(zl*zr);
                    //double t = acos(1-2*z);
                    double c = sin(t)/2;
                    double width = histos[i]->GetBinWidth(bin);
    ...
    // ratio plots are weird and i am lazy 
    int len = 600;
    if (ratio) {
        len = 800;
    }
    
    TCanvas* canvas1 = new TCanvas("c", "Comparison", 800, len);
    canvas1->cd();
        canvas1->SetLogy();
    ...
    */
    /*
        ...
        pad2->cd();
        rh = (TH1D*)EEC_w_p[0]->Clone();
        sm = (TH1D*)EEC_w_p[2]->Clone();
        //sm = (TH1D*)mbssignalfinal->Clone();
        rh->Divide(sm);
        
        rh->SetTitle("");
        rh->GetYaxis()->SetTitle("t1t1 / t2t2");
        //rh->GetYaxis()->SetTitle("EEC_theta /c * EEC_z");
        ...
        TGaxis *axis7 = new TGaxis(0, 0.95, bins/2, 0.95, .0001, .5, 510, "G"); 
        TGaxis *axis8 = new TGaxis(bins, 0.95, bins/2, 0.95, .0001 , .5 , 510, "-G");
...
*/
    ...
    for (int i = 0; i < number; ++i) {
        ...
        if (i == 2) {
            ...
            EEC_w_p[i]->SetLineColor(kOrange+2);
            EEC_w_p[i]->SetMarkerColor(kOrange+2);
        }
   ...
   // axis2->ChangeLabel(3, -1, 0, -1, -1, 62, "-" );
    ...
    TGaxis *axis7 = new TGaxis(0, 0, bins, 0, .001, M_PI , 2, "C"); 
    TGaxis *axis8 = new TGaxis(0, max, bins, max, .001, M_PI , 2, "-C");
    
    axis7->ChangeLabel(1, -1, 0, -1, -1, 62, "" );
    axis7->ChangeLabel(2, -1, -1, -1, -1, 62, "#pi/2" );
    axis7->ChangeLabel(3, -1, 0, -1, -1, 62, "" );
    ...
    if(delphi){
        ...
        */
    axis7->SetLabelOffset(0.020);
    axis8->SetLabelOffset(0.0025);
    axis7->SetLabelSize(.03);
    axis8->SetLabelSize(.03);
    axis7->SetTickSize(0.5);
    axis8->SetTickSize(0.5);
    ...
    if(delphi){
        ...
        drawText("#Delta#phi", 0.9, .03, 20);
    }
    else{
        drawText("0.5", 0.490, .06, 17 );
    ...
    //legend->AddEntry(EEC_w_p[0], "Pythia ", "pl");
    //legend->AddEntry(EEC_w_p[1], "Pythia EEC", "pl");
    //legend->Draw();
    ...
    //canvas2->SaveAs("CosSpectrum.png");
