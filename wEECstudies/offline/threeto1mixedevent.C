...
using namespace std;
void threeto1mixedevent() {
    
    TH1::SetDefaultSumw2();
    TH2::SetDefaultSumw2();
    const int file = 2;
    
    const char* filenames[file] = {
    ...
    //vector<const char*> histonames = {"EEC_wp", "EEC_tp", "EEC_mp", "EEC_sp", "EEC_s"};
    
    //vector<const char*> histonames = {"EEC_w_phi", "EEC_w_lowphi", "EEC_w_midphi", "EEC_w_hiphi"};
    
    bool rebin = 0;
    bool delphi = 0; 
    //bool mbs = 1; //mixed background subtraction
    ...
    string outputFilename = "../plots/tryJune20.C";
    string outputFilename1 = "../plots/tryJune20.png";
    int bins = 60;
    for (int i = 0; i < number; ++i) {
        
        for (i = number) {
            TH1F* htemp = dynamic_cast<TH1F*>(filenames[1]->Get(histonames[i]));
            ...
            if(rebin){
                ...
                TH1F* htemp = dynamic_cast<TH1F*>(filenames[0]->Get(histonames[i]));
                // to keep it separte and not mess up your root file
                histos[i] = (TH1F*)htemp->Clone(Form("EEC_clone_%d", i));
                histos[i]->SetDirectory(0);
        ...
        E2C_ME_cor = (TH1F*)EEC_w_p[0] ->Clone();
        E2C_ME_cor->Add(EEC_w_p[1], -1);
		E2C_ME_cor->Add(EEC_w_p[2], -1);
		E2C_ME_cor->Add(EEC_w_p[3], +1);
		E2C_cor->SetMarkerStyle(29);
		E2C_cor->SetMarkerColor(kOrange+1);
		E2C_cor->SetLineColor(kOrange+1);
    ...
    // horribly done very innefficient but it works :)
        ...
        if(mbs){
        legend->AddEntry(E2C_ME_corr, "Mixed background subtraction", "pl");
        }
