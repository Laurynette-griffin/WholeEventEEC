...
void mbs_standard() {
    ...
    TFile* file = TFile::Open("../outfiles/closuretrialztheta_oct27_2M.root");
    
    string outputFilename = "../plots/closuretrialz.C";
    string outputFilename1 = "../plots/closuretrialz.png";
    const char* pttrackcut = "charged constituent p_{T} > 0.2 GeV";
    
    const int number = 8; //! number of hists 
    
    int bins = 40;
    double min = 1; // initializing min and max for y axis because its easier to do right axis later.
    double max = 1e8;
    
    double min1 = 1e-1; // initializing min and max for y axis because its easier to do right axis later. maybe i should get the max entry and add an oom 
    double max1 = 1e1;

    //vector<const char*> histonames = {"EEC_w", "EEC_s", "EEC_m", "EEC_t","EEC_ms", "EEC_mm", "EEC_m2", "EEC_mt"};
    vector<const char*> histonames = {"EEC_wr", "EEC_sr", "EEC_mr", "EEC_tr","EEC_msr", "EEC_mmr", "EEC_m2r", "EEC_mtr"};
    //vector<const char*> histonames = {"EEC_wpl","EEC_spl", "EEC_mpl", "EEC_tpl", "EEC_mspl", "EEC_mmpl", "EEC_m2pl", "EEC_mtpl"};
    //vector<const char*> histonames = {"EEC_w", "EEC_s", "EEC_m", "EEC_t", "EEC_ms", "EEC_mm", "EEC_m2", "EEC_mt", 
       // "EEC_pt1", "EEC_pt2","EEC_pt3", "EEC_t1t1", "EEC_t2t2", "EEC_t3t3", "EEC_t1t2", "EEC_t1t3", "EEC_t2t3"};
    //vector<const char*> histonames = {"EEC_wpl","EEC_spl", "EEC_mpl", "EEC_tpl", "EEC_mspl", "EEC_mmpl", "EEC_m2pl", "EEC_mtpl"};
    //vector<const char*> histonames = {"EEC_wp","EEC_sp", "EEC_mp","EEC_tp", "EEC_msp", "EEC_mmp", "EEC_m2p"};

    bool delphi = 0;     //! delta phi x axis
    bool delr = 0;       //! delta r x axis
    bool theta = 0;      //! theta x axis
    bool integrate = 0;  //! integrate hists
    bool linear = 0;     //! linear binning
    bool extend = 0;     //! thermal hist checkinh
    bool scalefactor = 0;//! scale factor corrections for bkgsub 
    ...
    TH1D* sm;
        ...
        for (int i = 0; i < number; ++i) {
            ...
            for (int bin = 1; bin <= histos[i]->GetNbinsX(); ++bin) {
                ...
                histos[i]->SetBinContent(bin, content);
                }
        //mbs stuff only
        }
    ...
    // okay so EEC_w - EEC_ms - EEC_mm + EEC_m2 = EEC_ thats what this code does :

    mbssignal = (TH1D*)prehistos[0]->Clone(); //! these have not been normalized by bin width 
    ...
    for (int bin = 1; bin <= mbssignal->GetNbinsX(); ++bin) {
            ...
            mbssignal->SetBinContent(bin, content);
    ...
    mbssignalfinal->SetMarkerStyle(21);
	mbssignalfinal->SetMarkerColor(kOrange+2);
	mbssignalfinal->SetLineColor(kOrange+2);
    ...
    rh->SetTitle("");
    rh->GetYaxis()->SetTitle("wEEC/ wEEC_s");
    rh->GetYaxis()->SetTitleSize(0.1);
    rh->GetYaxis()->SetTitleOffset(0.4);
    ...
    //line->Draw();
    /*
    TGaxis *axis7 = new TGaxis(0, 0.85, bins/2, 0.85, .0001, .5, 510, "G"); 
    TGaxis *axis8 = new TGaxis(bins, 0.85, bins/2, 0.85, .0001 , .5 , 510, "-G");
  ...
  /*
    ...
    */
    ...
    //.00001, 4.957743);
    /*
    TGaxis *axis1 = new TGaxis(.00001, 0,4.957743, 0, .00001,4.957743, 510, ""); 
    axis1->SetLabelSize(.3);
...
;
    ...
    legend->AddEntry(EEC_w_p[1], "Signal + Signal", "pl");
    legend->AddEntry(EEC_w_p[2], "Signal + Thermal", "pl");
    legend->AddEntry(EEC_w_p[3], "Thermal1 + Thermal1", "pl");
    legend->AddEntry(EEC_w_p[4], "MixedE + T2", "pl");
    legend->AddEntry(EEC_w_p[5], "T2 + T2", "pl");
    legend->AddEntry(EEC_w_p[6], "T2 + T3", "pl");
    //legend->AddEntry(EEC_w_p[7], "Signal_{True} + Thermal_{Mixed}", "pl");
