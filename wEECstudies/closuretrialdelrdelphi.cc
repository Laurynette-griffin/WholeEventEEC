...
//allows for the .cmnd file to be accepted as an argument 
    ...
    Double_t topi[41] =   {1.000000000000e-05,1.818888151268e-05,3.308354106823e-05,6.017526085100e-05,1.094520689613e-04,
                            1.990810713656e-04,3.621062018486e-04,6.586306800430e-04,1.197975539992e-03,2.178983515200e-03,
                            3.963327297606e-03,7.208849061213e-03,1.311209014172e-02,2.384942539713e-02,4.337943726940e-02,
                            7.890234445798e-02,1.435145394419e-01,2.610368953255e-01,4.747969159514e-01,8.636024846826e-01,
                            1.570796326795e+00,
                            2.277990168907e+00,
                            2.666795737638e+00,2.880555758264e+00,2.998078114148e+00,3.062690309132e+00,3.098213216320e+00,
                            3.117743228193e+00,3.128480563448e+00,3.134383804529e+00,3.137629326292e+00,3.139413670075e+00,
                            3.140394678050e+00,3.140934022910e+00,3.141230547388e+00,3.141393572518e+00,3.141483201521e+00,
                            3.141532478329e+00,3.141559570049e+00,3.141574464708e+00,3.141582653590e+00};
    ...
    TH1::SetDefaultSumw2();
    TH2::SetDefaultSumw2();
    double madr = 4.957743;
    double midr = 0.00001;
    // whole event with thermal and mixed seperated out   
    TH1D EEC_wr("EEC_wr", "Energy Energy Correlator", bins, midr, madr); //! linear z 
    TH1D EEC_tr("EEC_tr", "Energy Energy Correlator", bins, midr, madr);
    TH1D EEC_mr("EEC_mr", "Energy Energy Correlator", bins, midr, madr);
    TH1D EEC_sr("EEC_sr", "Energy Energy Correlator", bins,  midr, madr);
    
    TH1D EEC_mtr("EEC_mtr", "Energy Energy Correlator", bins, midr, madr);
    TH1D EEC_msr("EEC_msr", "Energy Energy Correlator", bins, midr, madr); // mix signal
    TH1D EEC_mmr("EEC_mmr", "Energy Energy Correlator", bins, midr, madr); // mix1 mix1 
    TH1D EEC_m2r("EEC_m2r", "Energy Energy Correlator", bins, midr, madr); // mix1 mix1
    ...
    TH1D EEC_mtpl("EEC_mtpl", "Energy Energy Correlator", bins, 0, M_PI); // mix signal phi
    ...
    for (int iEvent = 0; iEvent < nEvt; ++iEvent) {
        ...
        for (size_t i = 0; i < charged_event.size(); ++i) {
            ...
            for (size_t k = 0; k < m1.size(); ++k) { // every k with the one i and this loops over i 
                ...
                double sm_z = charged_event.at(i).delta_R(m1.at(k));
                double sm_delphi = deltaphi(charged_event.at(i),m1.at(k));
                
                EEC_msr.Fill(sm_z, sm_eec/pmq2);
                EEC_mspl.Fill(sm_delphi,sm_eec/pmq2);
                if (ui == -1) {
                    EEC_mtr.Fill(sm_z, sm_eec/pmq2);
                    EEC_mtpl.Fill(sm_delphi,sm_eec/pmq2);
                }
            }
        ...
        for (size_t i = 0; i < m1.size(); ++i) {
            ...
            for (size_t k = i + 1; k < m1.size(); ++k) {
                //if (m1.at(k).pt() < 1.0) continue;
            double mm11_eec = m1.at(i).pt() * m1.at(k).pt();
            //double mm_ctheta = costheta(m1.at(i), m1.at(k));
            double mm_z = m1.at(i).delta_R(m1.at(k));
            double mm_delphi = deltaphi(m1.at(i),m1.at(k));
            
            EEC_mmr.Fill(mm_z, mm11_eec / pmq2);
        ...
        for (size_t i = 0; i < m1.size(); ++i) {
            ...
            for (size_t k = 0; k < m2.size(); ++k) {
                ...
                double mm12_eec = m1.at(i).pt() * m2.at(k).pt();
                double mm12_ctheta = costheta(m1.at(i),m2.at(k));
                double mm12_z = m1.at(i).delta_R(m2.at(k));
                double mm12_delphi = deltaphi(m1.at(i),m2.at(k));
                
                EEC_m2r.Fill(mm12_z, mm12_eec / pmq2); 
