       ...
       // pT cut > .2 GeV
        ...
        for (size_t i = 0; i < charged_event.size(); ++i) {
            for (size_t j = i + 1; j < charged_event.size(); ++j) {
                ...
                // work PLEASE! 
                if (jets[0].pt() < 31.2) EEC_w_low.Fill(z, eec/pmq2);
                if (jets[0].pt() >= 31.2 && jets[0].pt() < 40.7) EEC_w_mid.Fill(z, eec/pmq2);
                if (jets[0].pt() >= 40.7 && jets[0].pt() < 60.8) EEC_w_high.Fill(z, eec/pmq2);
