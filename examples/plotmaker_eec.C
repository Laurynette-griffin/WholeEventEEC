...
void plotmaker_eec() {
 ...
 // EEC_w_p.GetXaxis()->SetTitleOffset(999);
  auto axis18 = new TGaxis(-9, -0.8, -9, 0.8, -8, 8, 50510, "");
  axis18->SetTitle("axis18");
  axis18->SetTitleOffset(0.5);
  axis18->Draw();
 
  TGaxis *axis1 = new TGaxis(0, 10e-4, 25, 10e-4, .0001, .5, 510, "G"); 
  TGaxis *axis2 = new TGaxis(50, 10e-4, 25, 10e-4, .0001, 1, 510, "-G");
  axis1->SetTitle("z = 1/(cos(#Deltar))");
  axis1->Draw();
  axis2->Draw(); 
