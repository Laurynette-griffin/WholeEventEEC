...
void plotmaker_eec15() {
 ...
 TH1::SetDefaultSumw2();
  ...
  TFile* fin = TFile::Open("../offline/pythia_pp_rhic_15GeV_fullevent_10pthat60_March3.root");  
  ...
  TH1F* histq2act= (TH1F*)fin->Get("q2actual");
  TH1F* histq2est= (TH1F*)fin->Get("q2est");
  TH1F* histq2diff= (TH1F*)fin->Get("q2difference");
  ...
  TCanvas* canvas12 = new TCanvas("canvas12","Pythia q2", 800, 600);
  TCanvas* canvas13 = new TCanvas("canvas13","q2 est", 800, 600);
  TCanvas* canvas14 = new TCanvas("canvas14","q2 diff", 800, 600);
 ...
 // make this a function 
  ...
  //drawText("jet p_{T, leading} > 40 GeV", 0.20, 0.77, 18);
  drawText("44.75 < #hat{p}{T} < 45.25", 0.20, 0.79, 18);
  drawText("charged constituent p_{T} > .2 GeV", 0.20, .75, 18 );
  //drawText("MPI off, ISR off, Hadronization off", 0.20, .72, 18 );
  
  
  canvas1->SaveAs("WholeEventEEC45.png");
  ...
  TH1F EEC_w_pub3("EEC_w_pub3", "Energy Energy Correlator", 50, 0, 50);
  ...
  canvas8->SaveAs("WholeEventEECmostunbalanced.png");
  ...
  TH1F EEC_w_pb3("EEC_w_pb3", "Energy Energy Correlator", 50, 0, 50);
  ...
  canvas9->SaveAs("WholeEventEECleastbalanced.png");
  ...
  canvas11->SaveAs("Deltardistribution.png");
  
  canvas12->cd();
  //histq2act->SetStats(0);
  histq2act->GetXaxis()->SetTitle("q2");
  histq2act->Draw("P E1");
  drawText("p+p #srqt{s} = 200 GeV", 0.20, 0.77, 18);
  
  canvas12->SaveAs("Pythia8q2narrowpthat.png");
  
  canvas13->cd();
  //histq2est->SetStats(0);
  histq2est->GetXaxis()->SetTitle("1/#frac{jet p_{T, leading} + jet p_{T, subleading}}{2}}^2");
  histq2est->Draw("P E1");
  drawText("p+p #srqt{s} = 200 GeV", 0.20, 0.77, 18);
  
  canvas13->SaveAs("q2estnarrowpthat.png");
  
  canvas14->cd();
  //histq2diff->SetStats(0);
  histq2diff->GetXaxis()->SetTitle("q2 - 1/#frac{jet p_{T, leading} + jet p_{T, subleading}}{2}}^2");
  histq2diff->Draw("P E1");
  drawText("p+p #srqt{s} = 200 GeV", 0.20, 0.77, 18);
  
  canvas13->SaveAs("q2diffnarrowpthat.png");
